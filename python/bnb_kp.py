...
class bnb(knapsack):
    ...
    def branch_and_bound(self, final_sol):
        ...
        left_child = struc_sol()
        right_child = struc_sol()
        while (self.QueueSize != 0 or solution.bound <= current_best):
            state = self.removeMax() # pop from queue

            left_child = state.copy()
            right_child = state.copy()

            left_child.fixed += 1
            right_child.fixed += 1

            #one true one false
            left_child.solution_vec.insert(left_child.fixed ,True)
            right_child.solution_vec.insert(right_child.fixed , False)
            self.frac_bound(left_child,left_child.fixed)

            if(left_child.val>current_best):
                self.insert(left_child)
                if(left_child.fixed == self.Nitems - 1):
                    current_best = left_child.val
                    self.copy_array(left_child.solution_vec ,final_sol)

            self.frac_bound(right_child,right_child.fixed)

            if(right_child.val>current_best):
                self.insert(right_child)
                if(right_child.fixed == self.Nitems - 1):
                    current_best = right_child.val
                    self.copy_array(right_child.solution_vec ,final_sol)
